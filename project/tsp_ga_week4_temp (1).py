...
try:
    ...
    # 이하 main
    for t in range(1):
        table = getTable(r'C:\Users\myung\generic\project\input100.txt')
        # table = getTable('/Users/kimjonghyeon/Library/Mobile Documents/com~apple~CloudDocs/3-2/심화프로그래밍/tsp_ga/dataset/att48_d.txt')
        ...
        start = time.time()
        while 23000 <= f[bestIdx]: #9352
            if generation % 100 == 0:
                print(generation, " >> ", f[bestIdx])
            ...
            for i in range(0, 3, 1):
                ...
                #offspring = pmx(p[i1], p[i2], 15, 38)
                # offspring = orderCrossover(p[i1], p[i2], 15, 38)
        ...
        print("최종세대 : ", generation)
        print("최적해 : ", f[getBestIdx()])
        print()
except KeyboardInterrupt:
    pass
finally:
    axes[0].plot(x, y1, color='blue', marker=',', alpha=0.5, linewidth=0.5)
    axes[1].plot(x, y2, color='red', marker=',', alpha=0.5, linewidth=0.5)
